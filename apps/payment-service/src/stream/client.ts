...
export class RedisStreamClient {
  ...
  async connect(): Promise<void> {
    try {
      ...
      console.log('payment-service: connected to redis');
    } catch (err) {
      console.warn('payment-service: redis not reachable yet:', (err as Error).message);
    }
  }

  // TODO: XADD an event envelope onto `stream` (roadmap item: payment-service).
